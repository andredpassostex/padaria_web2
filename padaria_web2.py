...
for key in ["tela_selecionada", "submenu_selecionado", "mostrar_caixa", "mostrar_contas"]:
    if key not in st.session_state:
        st.session_state[key] = "Dashboard" if key=="tela_selecionada" else None if key=="submenu_selecionado" else False
...
def box_title(texto, icone="📌"):
    st.markdown(f"""
        <div style='padding: 10px; background-color: #f9f9f9;
                    ...
                    margin-bottom: 12px;'>{icone} <b>{texto}</b>
        </div>
...
def registrar_venda(produto, funcionario, cliente, quantidade, tipo="imediata"):
    ...
    if produto.qtd <= produto.estoque_min:
        st.warning(f"⚠ Estoque baixo: {produto.nome} ({produto.qtd})")
...
def dashboard():
    ...
    with col1:
        st.markdown(f"<div style='background:#4CAF50;padding:10px;color:white;text-align:center'><h4>💰 Caixa</h4><h2>{display_valor}</h2></div>", unsafe_allow_html=True)
        if st.button("👁️", key="btn_caixa"): st.session_state["mostrar_caixa"]=not st.session_state["mostrar_caixa"]
    with col2:
        st.markdown(f"<div style='background:#2196F3;padding:10px;color:white;text-align:center'><h4>🛒 Vendas Hoje</h4><h2>{len(vendas_hoje)}</h2></div>", unsafe_allow_html=True)
    with col3:
        lista = "<br>".join([f"{p.nome} ({p.qtd})" for p in produtos_baixos]) if produtos_baixos else "✅ OK"
        st.markdown(f"<div style='background:#FF9800;padding:10px;color:white;text-align:center'><h4>📦 Produtos Baixos</h4><p>{lista}</p></div>", unsafe_allow_html=True)
    with col4:
        display_conta = f"R$ {total_contas:.2f}" if st.session_state["mostrar_contas"] else "R$ ****"
        st.markdown(f"<div style='background:#F44336;padding:10px;color:white;text-align:center'><h4>👥 Contas em Aberto</h4><h2>{display_conta}</h2></div>", unsafe_allow_html=True)
        if st.button("👁️", key="btn_conta"): st.session_state["mostrar_contas"]=not st.session_state["mostrar_contas"]
...
def tela_funcional():
    ...
    if tela=="Estoque":
        if submenu=="Cadastrar Produto":
            ...
            nome = st.text_input("Nome Produto", key="cad_prod_nome")
            ...
            if st.button("Cadastrar Produto", key="btn_cad_prod"): cadastrar_produto(nome, qtd, preco)
    ...
    # ----------------- Funcionários -----------------
        ...
        if submenu=="Cadastrar Funcionário":
            ...
            nome = st.text_input("Nome", key="cad_func_nome")
            if st.button("Cadastrar Funcionário", key="btn_cad_func"): cadastrar_funcionario(nome)
            ...
            sel = st.selectbox("Selecione", nomes, key="rem_func")
            if st.button("Remover", key="btn_rem_func"): remover_funcionario(sel)
    ...
    # ----------------- Clientes -----------------
        ...
        if submenu=="Histórico":
            ...
            if st.session_state["clientes"]:
                ...
                sel_cliente = st.selectbox("Cliente", nomes_clientes, key="hist_sel_cliente")
                ...
                if cliente.historico:
                    ...
                    def color_tipo(tipo):
                        return ['background-color:orange' if tipo=='reserva' else 'background-color:lightgreen']*6
                    st.dataframe(df.style.apply(lambda x: color_tipo(x["Tipo"]), axis=1), use_container_width=True)
                else: st.info("Sem histórico")
            else: st.info("Nenhum cliente cadastrado")

        elif submenu=="Conta":
            box_title("Gerenciar Conta")
            if st.session_state["clientes"]:
                nomes = [c.nome for c in st.session_state["clientes"]]
                sel = st.selectbox("Cliente", nomes, key="conta_sel_cliente")
                ...
                hist = [x for x in cliente.historico if x[5]=="reserva"]
                if hist:
                    df = pd.DataFrame(hist, columns=["Produto","Qtd","Total","Data/Hora","Funcionário","Tipo"])
                    st.dataframe(df.style.apply(lambda x: ['background-color:orange']*6, axis=1), use_container_width=True)
                else: st.info("Sem reservas")
                if st.button("Zerar Conta", key="btn_zerar_conta"):
                    for x in cliente.historico:
                        if x[5]=="reserva": x[5]="pago"
                    st.success("Conta zerada")
            else: st.info("Nenhum cliente cadastrado")
    ...
    # ----------------- Fornecedores -----------------
        ...
        if submenu=="Cadastrar Fornecedor":
            ...
            nome = st.text_input("Nome", key="cad_forne_nome")
            contato = st.text_input("Contato", key="cad_forne_contato")
            produto = st.text_input("Produto", key="cad_forne_prod")
            preco = st.number_input("Preço", min_value=0.01, value=1.0, format="%.2f", key="cad_forne_preco")
            prazo = st.number_input("Prazo de entrega", min_value=0, value=0, key="cad_forne_prazo")
            if st.button("Cadastrar Fornecedor", key="btn_cad_forne"): cadastrar_fornecedor(nome, contato, produto, preco, prazo)
    ...
    # ----------------- Vendas -----------------
        ...
        if not st.session_state["produtos"] or not st.session_state["funcionarios"]:
            st.info("Cadastre produtos e funcionários primeiro")
        else:
            ...
            nomes_clientes = ["Novo Cliente"] + [c.nome for c in st.session_state["clientes"]]
            sel_cliente = st.selectbox("Cliente", nomes_clientes, key="venda_cliente_sel")
            cliente = None
            ...
            if st.button("Registrar Venda", key="btn_registrar_venda"):
                if sel_cliente=="Novo Cliente":
                    nome_novo = st.text_input("Nome do Cliente", key="venda_cliente_novo")
                    cliente = cadastrar_cliente(nome_novo) if nome_novo else None
                else:
                    cliente = next((c for c in st.session_state["clientes"] if c.nome==sel_cliente), None)
    ...
    # ----------------- Caixa / Relatórios -----------------
        ...
        if submenu in ["Diário","Semanal","Mensal"]:
            ...
            if st.session_state["vendas"]:
                ...
                if vendas_filtradas:
                    df = pd.DataFrame([[v[1],v[2],v[6].strftime("%d/%m/%Y %H:%M"),v[7],v[5]] for v in vendas_filtradas],
                                      columns=["Produto","Qtd","Data/Hora","Funcionário","Tipo"])
                    st.dataframe(df, use_container_width=True)
                    st.markdown(f"**Total Vendas ({submenu}): R$ {sum(v[4] for v in vendas_filtradas):.2f}**")
                else: st.info(f"Nenhuma venda {submenu.lower()}")
        else:
            box_title("Caixa")
            total_caixa = sum(v[4] for v in st.session_state["vendas"] if v[5]=="imediata")
...
for item,submenus in menu_expansivo.items():
    exp = st.sidebar.expander(item)
    with exp:
        for sub in submenus:
            if st.button(sub, key=f"{item}_{sub}"):
                st.session_state["tela_selecionada"] = "Caixa" if item=="Relatórios" else item
